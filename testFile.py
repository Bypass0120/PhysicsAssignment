...
class particle:
	def __init__(self,pos, vel, mass = 1.0, size = 10, shape = "circle", color = (0,0,255)):
		...
		self.cor = .93     #Coefficient of restitution
...
class rectangle(particle):
	def __init__(self, pos, vel, width=20, height=20, theta=0, omega=0, mass=1.0, shape="rectangle", color=(255, 0, 255)):
		...
		self.theta = theta
		self.omega = omega
		self.local_x_axis = vect3d(1,0,0)
		self.local_y_axis = vect3d(0,1,0)
		self.torque = vect3d(0,0,0)
		self.torque_old = vect3d(0,0,0)

	def draw(self, screen):
		pygame.draw.aalines(screen, self.color, True, self.find_corners())
		#pygame.draw.circle(screen, self.color, (int(self.pos.x), int(self.pos.y)), (int(self.size)), 1)
		#Draw the local x and y axis
		pygame.draw.aaline(screen, (255,0,0), [self.pos.x, self.pos.y], [self.pos.x + (rect.width/2 * self.local_x_axis).x, self.pos.y + (rect.width/2 * self.local_x_axis).y], True)
		pygame.draw.aaline(screen, (0,0,255), [self.pos.x, self.pos.y], [self.pos.x + (rect.height/2 * self.local_y_axis).x, self.pos.y + (rect.height/2 * self.local_y_axis).y], True)
	...
	def find_corners(self):
		temp_x = self.width/2*self.local_x_axis.norm()
		temp_y = self.height/2*self.local_y_axis.norm()
		corners = [[-(temp_x.x + temp_y.x) + self.pos.x, -temp_x.y - temp_y.y + self.pos.y],
					[temp_x.x - temp_y.x + self.pos.x, temp_x.y - temp_y.y + self.pos.y],
					[temp_x.x + temp_y.x + self.pos.x, temp_x.y + temp_y.y + self.pos.y],
					[-temp_x.x + temp_y.x  + self.pos.x, -temp_x.y + temp_y.y + self.pos.y]]
		return corners
	...
	def update_rotation(self, dt):
		...
		self.local_x_axis = vect3d(math.cos(self.theta * math.pi/180), -math.sin(self.theta * math.pi/180),0)
		self.local_y_axis = vect3d(math.sin(self.theta * math.pi/180), math.cos(self.theta * math.pi/180), 0)
...
if __name__ == '__main__': 
	...
	for i in range(num_part):
		if i < circle_num:
			r = vect3d(50 * math.cos(i * 2*math.pi/circle_num) + 200, 
				50 * math.sin(i * math.pi * 2 /circle_num) + 200, 0)
		else:
			r = vect3d(random.randint(0, earth.width), random.randint(0, earth.height), 0)
		v = vect3d(randint(-5,5),randint(-5,5), 0)
	...
	part = particle(vect3d(315, 240, 0), vect3d(0,8,0))
	part_list.append(part)
	earth.add_particle(part)

	rect = rectangle(vect3d(300, 300,0), vect3d(0,0,0), 50, 50,0)
