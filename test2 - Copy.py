...
class coil(rectangle):
	def __init__(self, pos):
		...
		for i in range(0,(int)(self.num_loops*20)):
			self.electron_path.append(vect2d(i*(self.width-5)/(20*self.num_loops) - self.width/2 + 5,
		...
		for i in range(0, 5):#Add 5 points along the wire on the right
			self.electron_path.append(vect2d(self.width-10 - self.width/2 + 5, 15 + i*(self.wire[1].height-15)/5))
		for i in range(0,7):#Add 7 points along the wire on the bottom
			self.electron_path.append(vect2d(self.wire[2].width-10 - i*(self.wire[2].width+10)/7 - self.width/2 + 5,
	...
	def induction(self, mag=1):
		for i in self.electrons:
			...
			nextIndex = -1
			if i.path_index + charge == len(self.electron_path):
				nextIndex = 0
			elif i.path_index + charge < 0:
				nextIndex = len(self.electron_path)-1
			else:
				nextIndex = i.path_index + charge

			#Find the direction the electron should go
			direction = self.electron_path[nextIndex] - self.electron_path[i.path_index]
			norm = direction.norm()

			#If moving the particle by mag along the normal
			#is greater than the distance to the next point,
			#place particle at next point and move the rest of the way along the normal to the next point\
			if (self.electron_path[nextIndex] - i.pos).mag() < (mag*norm).mag():
				i.pos = self.electron_path[nextIndex]
				i.path_index = nextIndex
			else:
				i.pos += mag * norm * charge
